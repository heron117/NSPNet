...
class RandomScaleAlbumentations:
    def __init__(self, scale=0.1, to_tensor=True, prob=0.5):
        self.scale = scale
    ...
    def __call__(self, image, targets=None):
        ...
        if targets is None:
            transform = A.Compose([
                ...
                A.PadIfNeeded(min_height=output_height, min_width=output_width, border_mode=cv2.BORDER_CONSTANT,
                              value=0),
            ...
            # 打印每个目标的详细信息
            # print("Original Targets Details:")
            # if isinstance(targets, BoxList):  # 如果是单个 BoxList
            #     print(f"  bbox: {targets.bbox}")
            #     print(f"  labels: {targets.get_field('labels')}")
            #     print(f"  size: {targets.size}")
            #     print(f"  im_info: {targets.extra_fields.get('im_info', 'No im_info available')}")
            # else:
            #     for idx, target in enumerate(targets):  # 遍历每个 BoxList 对象
            #         print(f"Target {idx}:")
            #         print(f"  bbox: {target.bbox}")
            #         print(f"  labels: {target.get_field('labels')}")
            #         print(f"  size: {target.size}")
            #         print(f"  im_info: {target.extra_fields.get('im_info', 'No im_info available')}")
            ...
            for idx, bbox in enumerate(transformed_bboxes):
                ...
                if xmax > xmin and ymax > ymin:
                    ...
                    valid_indices.append(idx)  # 记录有效检测框的索引

            # 检查裁剪后的边界框，如果没有有效边界框，则返回原始图像和目标
            if not clipped_bboxes:
                return Image.fromarray(image) if isinstance(image, np.ndarray) else image, targets

            # 创建新的目标对象
            transformed_target = BoxList(torch.tensor(clipped_bboxes), (output_width, output_height), mode="xyxy")

            # 保持 labels 不变，只根据原始标签的顺序重新添加它们
            if labels is not None:
                transformed_target.add_field("labels", torch.tensor([labels[idx] for idx in valid_indices]))
                #[labels[idx] for idx in valid_indices]提取有效标签  从原始labels列表中选择与有效边界框对应的标签
                #通过[labels[idx]获取每个有效边界框的标签,确保了裁剪后的边界框只对应有效的标签
                #tensor转换为张量,然后添加到transformed_target中作为一个字段
                #确保了在裁剪或缩放图像后，只保留有效的标签并与裁剪后的边界框顺序对齐，避免在训练中出现标签和边界框不匹配的情况。

            #
            # # 打印 transformed_target 的详细信息
            # print("Transformed Target Details:")
            # print(f"  bbox: {transformed_target.bbox}")
            # print(f"  labels: {transformed_target.get_field('labels')}")
            # print(f"  size: {transformed_target.size}")
            # print(f"  im_info: {transformed_target.extra_fields.get('im_info', 'No im_info available')}")
...
class GaussianBlur(object):  # 用于对图像加入高斯模糊
    def __init__(self, kernel_size=5, sigma=(0.1, 0.5), prob=0.5):
