...
class DualNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for stage_name in self.resnet.stages:
            ...
            # 动态获取当前ResNet输出的特征图大小
            target_size = x.shape[2:]  # 获取尺寸(H, W)

            # 使用自适应池化调整 wavelet_high 和 wavelet_low 的大小以匹配ResNet的输出
            wavelet_high = F.adaptive_avg_pool2d(wavelet_high, output_size=target_size)
            wavelet_low = F.adaptive_avg_pool2d(wavelet_low, output_size=target_size)

            # 检查形状，确保池化后的大小与ResNet输出匹配
