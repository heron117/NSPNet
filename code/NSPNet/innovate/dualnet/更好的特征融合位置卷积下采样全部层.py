...
class DualNet(nn.Module):
    def __init__(self, cfg):
        ...
        # 定义用于下采样的小波特征卷积
        self.downsample_conv_high = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1)  # high_freq卷积
        self.downsample_conv_low = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1)  # low_freq卷积
    ...
    def forward(self, x):
        ...
        for stage_name in self.resnet.stages:
            ...
            # 获取当前层的编号 (layer1, layer2, layer3, layer4)
            current_layer_index = int(stage_name[-1])  # 获取当前是第几层
            # 在每层进行相应次数的下采样
            for _ in range(current_layer_index):
                wavelet_high = self.downsample_conv_high(wavelet_high)
                wavelet_low = self.downsample_conv_low(wavelet_low)
            # 只在指定层进行融合操作
            if stage_name == 'layer2':  # 或者你可以根据需求在不同的层进行融合
                print(f"当前融合层 {stage_name}")

                # 检查形状，确保卷积下采样后的大小与ResNet输出匹配
                print(f"ResNet输出x的形状: {x.shape}")
