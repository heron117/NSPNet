...
#         self.high_freq_bn = nn.BatchNorm2d(64)  # 添加批处理归一化
#         self.high_freq_relu = nn.ReLU(inplace=True)  # 添加激活函数
#
#         self.low_freq_conv = nn.Conv2d(3, 64, kernel_size=3, padding=1)
#         self.low_freq_bn = nn.BatchNorm2d(64)  # 添加批处理归一化
#         self.low_freq_relu = nn.ReLU(inplace=True)  # 添加激活函数
#
#     def forward(self, x):
#         # 如果输入在 GPU 上，将其移动到 CPU 以便进行小波变换
#         if x.is_cuda:
#             x_cpu = x.cpu().detach().numpy()
#         else:
#             x_cpu = x.detach().numpy()
#
#         # 使用 PyWavelets 进行小波变换，获取高频和低频分量
#         coeffs2 = pywt.dwt2(x_cpu, 'haar')#这里只能接受单个样本的输入
#         LL, (LH, HL, HH) = coeffs2
#
#         # 将小波变换结果转回 PyTorch 张量，并移回 GPU（如果原输入在 GPU 上）
...
#         self.cbam_layers = nn.ModuleDict()  # 修改为 nn.ModuleDict
#
#     def forward(self, x):
#         outputs = []
...
#
#         # 打印 WaveletNet 输出的特征统计信息
#         print("Wavelet High Frequency Features - Mean: {:.4f}, Std: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(
#             wavelet_high.mean().item(), wavelet_high.std().item(), wavelet_high.max().item(), wavelet_high.min().item()))
...
#                 print(f"output ResNet x的形状: {x.shape}")
#                 print("output ResNet feature x- Mean: {:.4f}, Std: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(
#                     x.mean().item(), x.std().item(), x.max().item(), x.min().item()))
#
...
#                 print("Wavelet High Frequency Features Resized - Mean: {:.4f}, Std: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(
#                     wavelet_high.mean().item(), wavelet_high.std().item(), wavelet_high.max().item(),
#                     wavelet_high.min().item()))
#
#                 print("Wavelet Low Frequency Features Resized - Mean: {:.4f}, Std: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(
#                     wavelet_low.mean().item(), wavelet_low.std().item(), wavelet_low.max().item(),
#                     wavelet_low.min().item()))
#                 # 打印调整后的 Wavelet 特征统计信息
#
#                 # 动态调整 fusion 卷积层
...
#                 if stage_name not in self.cbam_layers:
...
#                 print(f"fused_feature - Mean: {fused_feature.mean().item():.4f}, Std: {fused_feature.std().item():.4f}, Max: {fused_feature.max().item():.4f}, Min: {fused_feature.min().item():.4f}")
#
#
    ...
    def forward(self, x):
        ...
        for stage_name in self.resnet.stages:
            ...
            if stage_name == 'layer2':
                ...
                # 动态调整卷积层，拼接 ResNet 和 wavelet 特征
                fused_feature = torch.cat([x, wavelet_high_resized, wavelet_low_resized], dim=1)
                fused_channels = fused_feature.shape[1]
                # print(f"fused_feature的形状: {fused_feature.shape}")
                # print(
                #     f"fused1_feature - Mean: {fused_feature.mean().item():.4f}, Std: {fused_feature.std().item():.4f}, Max: {fused_feature.max().item():.4f}, Min: {fused_feature.min().item():.4f}")

                # 动态生成 fusion_conv，如果还未创建或通道数不匹配
                # if stage_name not in self.fusion_convs or self.fusion_convs[stage_name].in_channels != fused_channels:
                #     self.fusion_convs[stage_name] = nn.Conv2d(fused_channels, x.shape[1], kernel_size=1).to(
                #         fused_feature.device)
                #     # 使用 Kaiming 正态初始化
                #     nn.init.kaiming_normal_(self.fusion_convs[stage_name].weight, nonlinearity='relu')
                #     if self.fusion_convs[stage_name].bias is not None:
                #         nn.init.constant_(self.fusion_convs[stage_name].bias, 0)
                if stage_name not in self.fusion_convs or self.fusion_convs[stage_name].in_channels != fused_channels:
                    self.fusion_convs[stage_name] = nn.Conv2d(fused_channels, x.shape[1], kernel_size=3, padding=1).to(
                        fused_feature.device)
                    # 使用 Kaiming 正态初始化
                    nn.init.kaiming_normal_(self.fusion_convs[stage_name].weight, nonlinearity='relu')
                    if self.fusion_convs[stage_name].bias is not None:
                ...
                fused_feature = self.fusion_convs[stage_name](fused_feature)

                fused_channels = fused_feature.shape[1]
                # print(f"fused_feature的形状: {fused_feature.shape}")
                # print(
                #     f"fused2_feature - Mean: {fused_feature.mean().item():.4f}, Std: {fused_feature.std().item():.4f}, Max: {fused_feature.max().item():.4f}, Min: {fused_feature.min().item():.4f}")

                # 动态生成 CBAM 注意力模块，如果还未创建或通道数不匹配  需要将fused_channels的通道数压缩到与x相同
                if stage_name not in self.cbam_layers:
                    self.cbam_layers[stage_name] = CBAM(fused_channels, x.shape[1]).to(fused_feature.device)
