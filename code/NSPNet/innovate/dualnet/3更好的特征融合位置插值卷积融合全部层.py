    ...
    def forward(self, x):
        ...
        for stage_name in self.resnet.stages:
            ...
            if stage_name == 'layer1':
                ...
                #     self.fusion_convs[stage_name] = nn.Conv2d(fused_channels, x.shape[1], kernel_size=1).to(fused_feature.device)
                if stage_name not in self.fusion_convs or self.fusion_convs[stage_name].in_channels != fused_channels:
                ...
                # 卷积操作，确保输入通道数与 fusion_conv 的输入通道数一致
                fused_feature = self.fusion_convs[stage_name](fused_feature)
                fused_feature = F.relu(fused_feature)  # 添加 ReLU 激活
                # print(f"outputs_feature的形状: {fused_feature.shape}")
                # print("fused_feature - Mean: {:.4f}, Std: {:.4f}, Max: {:.4f}, Min: {:.4f}".format(
                #     fused_feature.mean().item(), fused_feature.std().item(), fused_feature.max().item(), fused_feature.min().item()))
#需要过relu
